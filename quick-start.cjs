...
2️⃣ Préparer le déploiement Firebase
...
async function prepareDeployment() {
  ...
  try {
    ...
    console.log('2. Déployez sur Firebase : firebase deploy --only hosting');
    console.log('3. Configurez les variables d\'environnement');
    console.log('4. Déployez !');
    
    const deployChoice = await question('\nVoulez-vous déployer sur Firebase maintenant ? (oui/non) : ');
